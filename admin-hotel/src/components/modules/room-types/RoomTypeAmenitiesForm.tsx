...
import { setRoomTypeAmenities } from '@/api/rest/endpoints/catalog';
...
export function RoomTypeAmenitiesForm({
  roomTypeId,
  currentAmenityIds,
}: {
  roomTypeId: string;
  ...
  const mutation = useMutation({
    mutationFn: (values: Values) => setRoomTypeAmenities(roomTypeId, values.amenityIds),
    onSuccess: () => {
      invalidateGraphql(apollo, 'roomTypes.amenities');
  ...
  return (
    <Form form={form} onSubmit={form.handleSubmit((values) => mutation.mutate(values))}>
      <MultiSelectField<Values>
        name="amenityIds"
        label="Amenities"
        description="Shown on the room type's guest-facing page."
        options={options}
      />
      <FormActions>
