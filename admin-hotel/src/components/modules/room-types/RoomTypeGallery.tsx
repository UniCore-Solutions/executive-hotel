...
import { uploadRoomTypeImage, deleteMedia, setRoomTypeMedia, type MediaInput } from '@/api/rest/endpoints/catalog';
...
export function RoomTypeGallery({
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => {
      invalidateGraphql(apollo, 'roomTypes.media');
  ...
  const primaryMutation = useMutation({
    mutationFn: (id: string) => {
      ...
      return setRoomTypeMedia(roomTypeId, asInput(next));
    },
    onSuccess: () => {
      invalidateGraphql(apollo, 'roomTypes.media');
  ...
  async function onFileSelected(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    setUploading(true);
    try {
      await uploadRoomTypeImage(hotelId, roomTypeId, file, asInput(media), {
        altText: roomTypeName,
  ...
  return (
    <div className="space-y-4">
      ...
      {media.length === 0 ? (
        <EmptyState icon={ImagePlus} title="No photos yet" description="Upload the first photo for this room type." />
      ) : (
