...
Discovered during the 2026-08-27 investigation. **Nothing here was fixed** — recording
only. Severity is engineering impact, not product priority.
...
### F-A (= FRONTEND.md F-2) — Non-MAD currency mis-denominates the reservation ~11x
- **Problem:** the backend performs no FX conversion; `quote` echoes the client's
  `currencyCode` onto unconverted MAD figures. The UI converts for display (correct), but
  `createReservation`/`charge` then persist the **MAD number labelled as the display
  currency**.
- **Evidence:** live — `currencyCode` MAD/EUR/USD all return `totalAmount: 7290.27`.
  `BookingFlow.tsx:300,321` send `currencyCode: currency` and `amount: quote.total`.
  Switcher is in `Header.tsx:61` on every page; EUR/USD/GBP all exist in `currencies`.
- **Severity:** P0 — writes wrong money to the database.

### F-B (= FRONTEND.md F-1) — Declined payment leaves an orphaned booking; retry double-books
- **Problem:** `reservations.create()` commits a **confirmed** reservation and sells
  inventory before `charge()` runs. A retry regenerates the idempotency key, creating a
  second reservation and a second inventory decrement.
- **Evidence:** `BookingFlow.tsx:296-347`; `generateIdempotencyKey()` is called inside the
  submit handler. No compensating cancel; the backend has no `pending` reservation state.
- **Severity:** P0 — data integrity + inventory loss.

### F-C (= FRONTEND.md F-3) — Room-page availability uses the wrong stay
- **Problem:** all three `getStayRoom` call sites omit `checkout` and `rooms`;
  `catalog.stayInputOf()` defaults to a **one-night, one-room** stay.
- **Evidence:** `RoomDetails.tsx:196,214`, `BookingFlow.tsx:123`, `catalog.ts` `stayInputOf`.
- **Severity:** P0 — the availability badge and capacity gate can both be wrong.

### F-D (= FRONTEND.md F-4) — Post-payment confirmation re-prompts for the email
- **Problem:** `BookingFlow` redirects to `/confirmation?ref=…` without the email;
  `ConfirmationFlow` tries `myReservations`, which is `UNAUTHORIZED` for anonymous
  bookers, swallows it, and shows an email form.
- **Evidence:** `BookingFlow.tsx:342`, `ConfirmationFlow.tsx:69`; live: anonymous
  `myReservations` → `UNAUTHORIZED`.
- **Severity:** P0 (UX) — immediately after paying.

### F-E (= FRONTEND.md F-11) — Promo codes are broken end to end
- **Problem:** `/offers` advertises 5 codes that do not exist; `validatePromo` is called on
  pages that never hydrate the backend offer catalog; and an unknown code makes the
  backend **throw**, killing the entire quote rather than just skipping the discount.
- **Evidence:** live `/offers` emits `SUMMER2026 · STAY4PAY3 · BESTRATE · CORP10 ·
  WELCOME5`; the DB has `SPRING25 · MADINA15 · SUMMER10`. `ensurePricingSources()` is
  absent from `SearchResults.tsx` and `OffersGrid.tsx`. Live quote with `SUMMER2026` →
  `errors:[{code:VALIDATION}], data:null`.
- **Severity:** P1 — a whole marketing surface leads to a dead end.

### F-F — `Quote.valid` / `Quote.message` are dead on both sides
- **Problem:** the schema defines a soft-failure contract that the backend never uses —
  `PricingServiceImpl.java:183` hardcodes `valid=true` and every failure throws. The
  frontend's `else` branches (`RoomDetails.tsx:294`, `BookingFlow.tsx:207`) are therefore
  unreachable, and specific backend messages are replaced by a generic string.
  `Quote.promoMessage` exists in the Java DTO but is **not exposed in the schema**.
- **Severity:** P1 — root cause of the unhelpful errors in F-E.

### F-G — `gqlRequest` collapses NOT_FOUND into "check your connection"
- **Problem:** the backend returns `errors[] + data:{x:null}` for ordinary not-found cases;
  `gqlRequest` throws on any `errors[]`, so every `if (!result)` "not found" branch in
  `ReservationFlow`, `CheckinFlow`, `ConfirmationFlow` and `RoomDetails` is **dead code**
  and users get a misleading connectivity error that retrying never fixes.
- **Evidence:** live `reservation(input)` unknown ref → `NOT_FOUND` + `data.reservation:null`;
  `roomType(id)` unknown id → `NOT_FOUND` + `data.roomType:null`.
- **Fix reference:** `backoffice-hotel/src/lib/api.ts` already reads
  `extensions.code` correctly.
- **Severity:** P1.
...
### B2 — `stay_x_pay_y` promotions throw
- **Evidence:** `PricingServiceImpl.java:293` → `DomainException.validation("stay_x_pay_y
  promos are not supported yet")`. The enum value is nonetheless persistable.
- **Severity:** Medium — an admin can create a promotion that makes every quote fail.
